...
운영된다고 가정한 시뮬레이션 데이터를 사용했다. 이번에는 **실제로 지금 수집 가능한
진짜 데이터**로 같은 미션을 다시 수행한다. 성수동은 EventHub의 핵심 상권이라, 이
지역에 대한 대중의 실제 관심도 흐름을 아는 것은 사업 배경 데이터로서 직접적인 가치가
있다. 비교 대상으로 '팝업스토어'(성수동의 대표 콘텐츠)와 '할인'(EventHub의 핵심
가치)을 함께 조회해, 세 키워드가 어떻게 얽여 움직이는지도 함께 살펴본다.
...
- **결측치**: 365일 전체에 결측 없음. 날짜 공백(빠진 날짜)도 0일 — API가 매일
  데이터를 안정적으로 제공했다.
- **이상치**: IQR(사분위범위) 1.5배 기준으로 성수동 10건, 팝업스토어 5건, 할인 3건이
  범위 밖으로 확인됐다. 이 값들은 측정 오류가 아니라 실제 사건(연휴, 특정 이슈 등)을
  반영할 가능성이 높다고 판단해 **제거하지 않고 그대로 분석에 포함**했다.
...
세 키워드 모두 대체로 완만한 흐름을 보이는 가운데, '성수동'만 **2026년 5월 초에
극단적인 단기 급등**을 보인다. '팝업스토어'는 성수동과 비슷한 시기에 소폭 동반
상승하지만 진폭은 훨씬 작다.
...
토요일이 20.3으로 가장 높고 화·수요일이 17.1대로 가장 낮다. **주말 평균이 평일
대비 약 1.13배** 높다.
...
5월이 23.4로 가장 높고 3월이 14.3으로 가장 낮다. 봄(5월)과 가을(9~11월, 19~21대)에
관심도가 높고 겨울~초봄(1~4월)에 낮아지는 계절 패턴이 보인다.

### 5-4. 키워드 간 상관관계

![상관관계](images/04_correlation_heatmap.png)

성수동-팝업스토어 상관계수 0.329(약한 양의 상관), 성수동-할인 상관계수 -0.001
(거의 무상관).

### 5-5. [보너스: 시계열 분해] 추세 / 주간 계절성 / 잔차

![시계열 분해](images/05_decomposition.png)

`statsmodels.seasonal_decompose`(additive, period=7)로 분해한 결과, 뚜렷한 주간
계절성(진폭 3.23)이 확인된다. 5월 초 급등은 추세(Trend) 성분에도 크게 반영되고,
잔차(Residual)에서도 그 시점에만 유독 큰 값(+53)이 튀는데, 이는 **정규 주간 패턴으로
설명되지 않는 이례적 사건**이라는 뜻이다.

### 5-6. [보너스: 베이스라인 예측] Seasonal-Naive vs 실측

![베이스라인 예측](images/06_forecast_baseline.png)

"지난주 같은 요일 값을 그대로 쓰는" 가장 단순한 계절성 예측을 최근 14일 테스트
구간에 적용했다. **MAE = 3.98, MAPE = 27.0%**로, 요일 패턴은 어느 정도 맞추지만
정교하지는 않다.

> **해석**: 성수동 검색량은 주간 패턴(요일 효과)이 분명히 존재하지만, 그보다 더 큰
> 변동(5월 급등 같은)은 요일 패턴만으로는 예측할 수 없는 불규칙한 이벤트성 변화이기
> 때문으로 보인다. 이는 예측 기법 자체의 결함이라기보다, 이 시계열에 "요일성 계절
> 패턴"과 "불규칙 이벤트성 변동"이라는 서로 다른 두 가지 성질이 섞여 있다는 뜻이다.
...
- **관찰**: 5/1 원값 100(정규화 기준 최댓값), 7일 이동평균도 그 주에 46까지 치솟아
  평소(15~20대)의 2배 이상. 시계열 분해 잔차에서도 이 시점에만 +53까지 튐.
...
- **사용 작업**: (1) NAVER API HUB 인증 오류("A subscription to the API is
  required") 원인 진단 및 올바른 엔드포인트(`naverapihub.apigw.ntruss.com`) 확인,
  (2) JSON→CSV 정제 스크립트 작성, (3) 시각화 6종 스타일링 코드, (4) 시계열 분해·
  베이스라인 예측 코드, (5) 인사이트 문장 초안 다듬기.
- **사용 이유**: NAVER API가 2026-07-31에 Developers Center에서 NAVER API HUB로
  이관되며 엔드포인트·인증 헤더가 바뀐 것을 확인하는 데 최신 정보 검색이 필요했고,
  반복적인 pandas 집계/시각화 보일러플레이트 작성 시간을 절감하기 위해 사용했다.
- **검증 방법**: (a) 실제 API 호출로 받은 원본 JSON을 코드로 직접 재실행해 수치가
  그대로 일치하는지 확인, (b) 결측치·날짜 공백·이상치를 전수 검사해 데이터 품질
  확인, (c) 상관계수·계절성 진폭 등 통계치를 요일/월/키워드 간 여러 각도에서 교차
  확인해 해석이 일관되는지 재검토했다.
