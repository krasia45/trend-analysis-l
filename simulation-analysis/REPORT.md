...
> 중심으로 해석해야 한다 (§7 한계점 참고). 단, §4의 "공급 공백기" 발견은 시뮬레이션이 아닌
> **원본 카탈로그 자체에서 나온 실제 사실**이다.

> **데이터 출처/라이선스 주의**: 원본 카탈로그의 브랜드명·할인 조건은 EventHub 서비스
> MVP 시연용으로 구성된 콘텐츠이며, 이미지 URL(`picsum.photos`)은 개발용 플레이스홀더로
> 실제 브랜드의 공식 페이지가 아니다. 자세한 내용은 README.md §11 참고.
...
확인했다. 특히 §4에서 확인한 '6일간의 공급 공백기'는 시뮬레이션이 아닌 실제 데이터에서
...
> 어디서부터는 여전히 시뮬레이션의 한계인지를 구체적으로 검증했다.
...
253일(2025-08-21~2026-04-30)을 실제 데이터의 경험적 분포(카테고리별 할인율 분포·진행
기간 분포·소상공인 비율·실제 브랜드 풀)로 통계적으로 백필해 총 **401건·365일**로
확장한 시나리오를 별도 구축했다. 신규 이벤트 유입률은 초기 0.5건/일에서 실제 구간
진입 시점 관측값(1.43건/일)까지 선형 증가하도록 설계했다(초기 스타트업의 이벤트
소싱 성장 곡선이라는 가정, 코드: `scripts/simulate_extended_catalog.py`).

![1년 전체 추이](images/extended/e01_full_year_trend.png)

**가설 검증**: §5-8에서 제기한 가설("신규 이벤트가 지속적으로 유입되면 예측이 더
안정적일 것")을 3-way로 공정하게 테스트했다 — ① 원본(112일) 말단, ② 확장(365일)
말단(백필해도 8월 공급단절은 그대로), ③ 확장(365일) 중간 구간(연속 공급 보장).
...
|---|---|
| ① 원본(112일) 말단 | 86.4% |
| ② 확장(365일) 말단 — 단절 여전 | 89.9% |
| ③ 확장(365일) 중간 — 연속 공급 | **24.5%** |

②는 ①과 거의 동일(오히려 소폭 악화)하지만 ③은 크게 개선된다. **"데이터가 많으면
예측이 좋아진다"가 아니라 "공급이 끊기지 않는 구간에서만 예측이 안정적이다"**가 정확한
결론이다 — 단순 표본 확대가 아니라 데이터의 구조적 연속성이 핵심이라는, 원래 가설보다
한 단계 더 정교해진 인사이트를 얻었다.

**월별 추이** (이제 가능해진 계절 관찰):

![월별 조회수](images/extended/e02_monthly_views.png)

**카테고리 트렌드 안정성 비교** (원본 28일 창 vs 확장 90일 창):

![카테고리 트렌드 안정성](images/extended/e04_trend_stability.png)

**정직한 한계**: 백필 구간은 통계적 시뮬레이션이며 실측이 아니다. 성장 곡선(0.5→1.43건/일)
가정은 검증되지 않았다. 카테고리 트렌드 비교(원본 28일 창 vs 확장 90일 창)는 비교 구간
길이가 달라 엄밀한 노이즈 통제 실험이 아니며, "작은 표본 스냅샷의 트렌드는 관측 시점에
따라 크게 흔들린다"는 정성적 근거로만 해석해야 한다. 전체 코드와 3-way 비교 재현은
`scripts/simulate_extended_catalog.py`, `scripts/build_extended_scenario.py`,
`scripts/analyze_extended_scenario.py` 참고 (이 확장 시나리오는 `analysis.ipynb`에는
포함되어 있지 않고 별도 스크립트로만 재현 가능하다).
