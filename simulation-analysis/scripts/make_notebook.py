...
감쇠, 카테고리별 트렌드 로테이션이라는 세 가지 뚜렷한 시계열 구조를 확인했다. 특히 §4에서 확인한
'6일간의 공급 공백기'는 시뮬레이션이 아니라 실제 카탈로그 데이터에서 나온 발견으로, 서비스 오픈 전
지금 바로 반영 가능한 실행 인사이트다. 이 구조를 실제 서비스 오픈 후 관측 데이터로 재현·검증할 수
있다면, "지금 뜨는 이벤트/카테고리"를 브랜드·소상공인에게 제공하는 EventHub의 핵심 인사이트 상품
(실시간 인기 랭킹)의 데이터 기반을 마련할 수 있다.
